...
function getapproved($user){
    ...
    $sql = "SELECT 
            ...
            documents_submitted.name,documents_submitted.doc_url FROM `documents_submitted`,`student`,`approval`,`hod`,`cc` WHERE
            documents_submitted.roll=student.roll
...
function getunapproved($user){
    ...
    $sql = "SELECT 
        ...
        $result = $conn->query($sql);  
    while ($row = $result -> fetch_assoc()){  
        if($row['hod_dept']==$row['stud_dept'] && $user==$row['hodname'] && $row['stud_section']!="" && $row['hod']=='' ){       
            array_push($data1, $row);
        }
        else if($row['cc_dept']==$row['stud_dept'] && $user==$row['ccname'] && $row['CC']==''){       
            array_push($data1, $row);
        }
...
function getstudentapproval($roll){
	include '../backend/conn.php';
	$sql = "SELECT * FROM approval WHERE roll='$roll'";
    $result = $conn->query($sql);
    if($result){     
           $row = $result -> fetch_assoc();
    }
